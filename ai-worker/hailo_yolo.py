...
class HailoYolo:
    """Chay YOLOv8s tren NPU Hailo-8L (13 TOPS) thay vi CPU.

    Model .hef nay da co NMS-by-class bake san tren chip (xem hailortcli
    parse-hef): output la 1 list 80 phan tu (1 per COCO class), moi phan tu
    la mang [x1,y1,x2,y2,score] da normalize 0-1. Preprocess (letterbox,
    khong doi mau BGR->RGB) va cong thuc denormalize duoc copy nguyen tu
    vi du chinh thuc cua Hailo cho dung model nay (hailo-apps repo,
    object_detection_post_process.py) - da test thuc te tren may nay.
    """

    def __init__(self, hef_path=DEFAULT_HEF_PATH, classes=None, conf=0.10):
